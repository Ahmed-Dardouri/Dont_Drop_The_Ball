...
func test_meter_drains_over_time() -> void:
	...
	# Skip past the pause period (0.2 seconds)
	ComboManager._process(0.2)
...
func test_add_orb_score_sets_pause_timer() -> void:
	ComboManager.add_orb_score(10)
	assert_almost_eq(ComboManager._drain_pause_timer, 0.2, 0.001, "Adding orb score should set pause timer")


func test_drain_paused_during_pause() -> void:
	ComboManager.add_orb_score(10)
	# Verify drain is paused
	assert_almost_eq(ComboManager._drain_pause_timer, 0.2, 0.001, "Pause timer should be set after orb collection")
	# Process during pause - should not drain
	ComboManager._process(0.1)
	assert_almost_eq(ComboManager._drain_pause_timer, 0.1, 0.001, "Pause timer should decrement by 0.1 seconds")
...
func test_drain_resumes_after_pause() -> void:
	...
	# Process during pause - should not drain
	ComboManager._process(0.1)
	assert_almost_eq(ComboManager._drain_pause_timer, 0.1, 0.001, "Pause timer should be 0.1 after 0.1 seconds")
	# Process after pause ends - drain should happen
	ComboManager._process(0.1)
	assert_almost_eq(ComboManager._drain_pause_timer, 0.0, 0.001, "Pause timer should be zero after 0.2 seconds total")
	assert_almost_eq(ComboManager.get_meter_value(), 9.9, 0.1, "Meter should drain after pause ends")


func test_multiple_orb_scores_extend_pause() -> void:
	ComboManager.add_orb_score(10)
	ComboManager.add_orb_score(15)
	assert_almost_eq(ComboManager._drain_pause_timer, 0.2, 0.001, "Pause timer should be reset after each orb collection")


func test_pause_duration_is_correct() -> void:
	assert_eq(ComboManager.DRAIN_PAUSE_DURATION, 0.2, "Drain pause duration should be 0.2 seconds")
