...
## Manages mode start/end and high scores.
...
## PLAY MODE configuration (beginner/endless).
##
...
signal mode_started(mode_id: int)

## Emitted when a mode ends with result (win: bool, score: int, etc.)
signal mode_ended(mode_id: int, result: Dictionary)
...
## Mode configs assigned in the editor. Keyed by mode_id at runtime.
@export var modes: Array[ModeConfig] = []
...
## Available mode configurations keyed by mode_id
...
func start_mode(mode_id: int) -> void:
	if not _available_modes.has(mode_id):
		push_warning("ModeManager: Cannot start unknown mode: %d" % mode_id)
		return
...
func _instantiate_mode_implementation() -> void:
	...
	if current_mode.implementation == null:
		push_warning("ModeManager: No implementation script for mode: %d" % current_mode.mode_id)
		return
	...
	if _mode_impl == null:
		push_warning("ModeManager: Failed to instantiate implementation for mode: %d" % current_mode.mode_id)
		return
...
func get_mode_config(mode_id: int) -> ModeConfig:
	if not _available_modes.has(mode_id):
		return null
...
func get_high_score(mode_id: int) -> int:
...
func set_high_score(mode_id: int, score: int) -> void:
