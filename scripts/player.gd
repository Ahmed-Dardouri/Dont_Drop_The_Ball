...
func check_ball_collision():
	# After moving, push any RigidBody2D we hit
	for i in get_slide_collision_count():
		var col := get_slide_collision(i)
		var rb := col.get_collider()
		if rb is RigidBody2D:
			var norm := col.get_normal()                        # normal pointing *out* of player
			var rel_v = velocity - rb.linear_velocity        # relative motion
			
			# Build an impulse along the contact normal and add inherited player speed
			var impulse = norm * header_strength + velocity * inherit_factor
			print(impulse)
			rb.apply_force(-1 * impulse, col.get_position() - rb.global_position)
